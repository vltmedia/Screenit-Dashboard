...
const ProjectsMenuList = (props) => {
  const dispatch = useDispatch();

  // let clientt;
  const clientttt = useSelector(state => state.screenit.projectsInfo);
  console.log(props);
...
<MenuItem value={number.Name}>{number.Name}</MenuItem>);
  ...
  function handleChange(e){
    ...
    console.log(findd);
    console.log(headerr);
    axios.get('http://67.205.187.138:1340/projects/'+ findd.id.toString(),headerr).then(({data}) => {
      dispatch({type: SCREENIT_PROJECT, payload: data});
      console.log(data);
  ...
  return (
    ...
    <InputLabel htmlFor="age-simple">Projects</InputLabel>
    <br></br>

    <Select id="clientselect" defaultValue={'DEFAULT'}
            value={ useSelector(state => state.screenit.selectedProjectInfo.Name)}
